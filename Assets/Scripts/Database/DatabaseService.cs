...
namespace FightDojo.Database
{
  ...
  public class DatabaseService : IDatabaseService
  {
    ...
    {
      persistentPath = Path.IsPathRooted(dbName)
        ? dbName
        : Path.Combine(Application.persistentDataPath, dbName);
      return TryInitializeDatabase(persistentPath);
    }
    ...
    {
      return connection.Table<Character>()
    ...
    {
      return connection.Table<Combos>()
    ...
    {
      return connection.Insert(newGame);
    }

    public int AddCharacter(Character newCharacter)
    {
      return connection.Insert(newCharacter);
    }

    public int AddCombo(Combos newCombo)
    {
      return connection.Insert(newCombo);
    }
    ...
    {
      connection.Delete<Character>(id);
    }

    public void DeleteCombo(int id)
    {
      connection.Delete<Combos>(id);
    }

    public Game GetGame(int id) =>
      connection.Find<Game>(id);

    public Character GetCharacter(int id) =>
      connection.Find<Character>(id);

    public Combos GetCombo(int id) =>
      connection.Find<Combos>(id);
    ...
    public List<ComboWithCharacter> GetCombosWithCharacterName(int gameId)
    {
      ...
      return connection.Query<ComboWithCharacter>(sql, gameId);
    }

    public void UpdateGameName(int id, string newName)
    {
      var game = connection.Find<Game>(id);
      if (game == null)
      {
    ...
    {
      var character = connection.Find<Character>(id);
      if (character == null)
      {
    ...
    {
      var combo = connection.Find<Combos>(id);
      if (combo == null)
      {
    ...
    public bool ExportDatabase(string exportPath)
    {
      ...
      try
      {
        ...
        byte[] dbBytes = connection.Serialize();
    ...
    public bool TryMergeDatabases(string secondDbPath)
    {
      ...
      try
      {
        ...
        connection.Execute("ATTACH DATABASE ? AS db2;", secondDbPath);
        ...
        connection.RunInTransaction(() =>
        {
          ...
          try
          {
            connection.Execute("ALTER TABLE Game ADD COLUMN OldId INTEGER;");
          }
          catch
          {
            // Уже существует
          ...
          try
          {
            connection.Execute("ALTER TABLE Character ADD COLUMN OldId INTEGER;");
          }
          catch
          {
            // Уже существует
          ...
          connection.Execute("ALTER TABLE Game DROP COLUMN OldId;");
          connection.Execute("ALTER TABLE Character DROP COLUMN OldId;");
        });
      }
      catch (Exception e)
      {
      ...
      finally
      {
        try
        {
          connection.Execute("DETACH DATABASE db2;");
        }
        catch (Exception e)
        {
          Debug.LogError(e.Message);
    ...
    public bool TryOpenDatabase(string path)
    {
      ...
      SQLiteConnection tempConnection = connection;
